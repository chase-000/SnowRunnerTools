...
namespace PakTool {
	...
	public sealed class Crc32Managed : HashAlgorithm {
		...
		private static readonly uint[] __CrcTable = new uint[] {
		...
		protected override void HashCore ( byte[] array , int ibStart , int cbSize ) {
			unchecked {
				m_CrcValue ^= DefaultSeed;
				while ( --cbSize >= 0 ) {
					m_CrcValue = __CrcTable[( m_CrcValue ^ array[ibStart++] ) & 0xFF] ^ ( m_CrcValue >> 8 );
				}
				m_CrcValue ^= DefaultSeed;
			}
		}
		...
		protected override byte[] HashFinal () {
			unchecked {
				HashValue = new byte[] { (byte) ( ( m_CrcValue >> 24 ) & 0xFF ) , (byte) ( ( m_CrcValue >> 16 ) & 0xFF ) , (byte) ( ( m_CrcValue >> 8 ) & 0xFF ) , (byte) ( m_CrcValue & 0xFF ) };
				return HashValue;
			}
		}
